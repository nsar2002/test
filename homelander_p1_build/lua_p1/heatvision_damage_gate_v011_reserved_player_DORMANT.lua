...
HOMELANDER_DAMAGE_ONESHOT_RESERVED = false
HOMELANDER_DAMAGE_ATTEMPT = 0
HOMELANDER_DAMAGE_LAST_TARGET = nil
HOMELANDER_DAMAGE_LAST_PLAYER = nil
...
function Homelander_HeatVisionDamageOneShotV011()
    -- A void native damage call may mutate and THEN throw. Never retry a
    -- previously submitted/ambiguous one-shot as if it were definitely absent.
    if HOMELANDER_DAMAGE_ONESHOT_RESERVED == true then
        log("REFUSED: one-shot damage already submitted/ambiguous in this Lua state")
        return false
    end
    local previous = HOMELANDER_DAMAGE_ATTEMPT
    if type(previous) ~= "number" or previous ~= previous or
       ...
       previous ~= math.floor(previous) then
        log("REFUSED: invalid/overflowed damage attempt generation")
        return false
    end
    local attempt = previous + 1
    HOMELANDER_DAMAGE_ATTEMPT = attempt
    ...
    if HOMELANDER_DAMAGE_ATTEMPT ~= attempt or HOMELANDER_DAMAGE_ONESHOT_RESERVED == true or
       HOMELANDER_PLAYER ~= player then
        log("REFUSED: newer damage attempt or player switch during camera/LOS callbacks")
        return false
    end
    ...
    -- Final validation AFTER the target's go_IsValid callback, which can
    -- synchronously run another F17, rebind player/target or revoke F16 proof.
    if HOMELANDER_DAMAGE_ATTEMPT ~= attempt or HOMELANDER_DAMAGE_ONESHOT_RESERVED == true or
       HOMELANDER_PLAYER ~= player or HOMELANDER_DAMAGE_ONESHOT_PASSED == true or
       HOMELANDER_DYNAMIC_AIM_STABILITY_PASSED ~= true or
       HOMELANDER_LASERSIGHT_SHADER_PASSED ~= true or
       not valid_goh(player) or not valid_goh(target) or
       HOMELANDER_PLAYER ~= player or HOMELANDER_DAMAGE_ATTEMPT ~= attempt or
       HOMELANDER_DAMAGE_ONESHOT_RESERVED == true then
        log("REFUSED: F17 player/target/stability/attempt invalidated before damage")
        return false
    end
    local stillNative, stillNativeOK = pcall(HL_LaserSightDynamicStabilityProbe)
    if not stillNative or stillNativeOK ~= true or
       HOMELANDER_PLAYER ~= player or HOMELANDER_DAMAGE_ATTEMPT ~= attempt or
       HOMELANDER_DAMAGE_ONESHOT_RESERVED == true then
        log("REFUSED: native dynamic stability or provenance invalidated before damage")
        return false
    end

    -- Reserve BEFORE the potentially mutating C binding: a nested F17 called
    -- by the native bridge must not submit a second damage event.
    ...
    if not okDamage then
        -- The bridge may have applied damage before throwing; an ambiguous
        -- submission remains consumed, but NEVER creates a successful F18 token.
        log("FAIL: damage submission outcome unknown; attempt consumed | " .. tostring(damageResult))
        return false
    end

    HOMELANDER_DAMAGE_LAST_TARGET = target
    HOMELANDER_DAMAGE_LAST_PLAYER = player
