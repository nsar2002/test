...
local function current_gate_state()
    ...
    -- F14 must be the most recent successful native one-shot for the SAME
    -- F9/F10/F11 generations, not just for the same named NPC/vehicle.
    local f14attempt = HOMELANDER_LASERSIGHT_ONESHOT_ATTEMPT
    ...
    if type(f14attempt) ~= "number" or f14attempt < 1 or
       type(f9epoch) ~= "number" or f9epoch < 1 or
       type(f10attempt) ~= "number" or f10attempt < 1 or
       type(f11epoch) ~= "number" or f11epoch < 1 or
       HOMELANDER_LASERSIGHT_ONESHOT_F9_EPOCH ~= f9epoch or
       HOMELANDER_LASERSIGHT_ONESHOT_F10_ATTEMPT ~= f10attempt or
       HOMELANDER_LASERSIGHT_ONESHOT_F11_EPOCH ~= f11epoch or
       HOMELANDER_FREEAIM_EPOCH ~= f9epoch or
       HOMELANDER_FREEAIM_F10_EPOCH ~= f9epoch or
       HOMELANDER_FREEAIM_F10_ATTEMPT ~= f10attempt or
       HOMELANDER_EYE_ORIGIN_EPOCH ~= f11epoch then
        return nil, "F14 success is not tied to current exact F9/F10/F11 proof"
    end
...
function Homelander_LaserSightHeldTickV009()
    ...
    if not finite_number(max_frames) then
        HOMELANDER_LASERSIGHT_HELD_STABILITY_PASSED = false
        HOMELANDER_LASERSIGHT_HELD_STABILITY_TARGET = nil
        disable("nonfinite held-render frame limit; no native submit")
        return false
    end
    ...
    if G.target ~= S.target or G.f14_attempt ~= S.f14_attempt then
        disable("target or one-shot attempt changed since held mode was armed")
        return false
    end
