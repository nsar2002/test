...
local function clear_eye_proof()
    HOMELANDER_EYE_ORIGIN_NAME = nil
    HOMELANDER_EYE_ORIGIN = nil
    HOMELANDER_EYE_ORIGIN_VERIFIED = false
    HOMELANDER_EYE_ORIGIN_VERIFIED_EPOCH = nil
    HOMELANDER_EYE_ORIGIN_VERIFIED_PLAYER = nil
    HOMELANDER_DUAL_EYE_RENDER_LAST_OK = false
    HOMELANDER_DUAL_EYE_RENDER_LAST_TARGET = nil
end

function Homelander_EyeOriginProbeV006()
    local previous = HOMELANDER_EYE_ORIGIN_EPOCH
    ...
    if type(previous) ~= "number" or previous ~= previous or
       ...
       previous ~= math.floor(previous) then
        clear_eye_proof()
        log("ABORT: invalid/overflowed F11 eye-measurement generation")
        return false
    end
    local attempt = previous + 1
    HOMELANDER_EYE_ORIGIN_EPOCH = attempt
    clear_eye_proof()
    log("BEGIN — READ ONLY | generation=" .. tostring(attempt))
    ...
    -- The optional player-position and validity callbacks may rebind the active player.
    -- Reject the entire F11 proof if its original subject changed at any point.
    if HOMELANDER_EYE_ORIGIN_EPOCH ~= attempt then
        log("ABORT: a newer F11 eye measurement superseded this callback")
        return false
    end
    if HOMELANDER_PLAYER ~= player or not valid_goh(player) or HOMELANDER_PLAYER ~= player then
        log("ABORT: originating player changed during F11 callbacks")
        return false
    end
    if HOMELANDER_EYE_ORIGIN_EPOCH ~= attempt then
        log("ABORT: nested F11 during final player validity; newer eye retained")
        return false
    end
    ...
    HOMELANDER_EYE_ORIGIN_VERIFIED_EPOCH = attempt
