...
local function bound_to_current(player, target, offset, f9epoch, f11epoch)
    if type(f11epoch) ~= "number" or f11epoch < 1 or
       ...
       HOMELANDER_FREEAIM_F10_EPOCH ~= f9epoch or
       HOMELANDER_EYE_ORIGIN_EPOCH ~= f11epoch or
       HOMELANDER_EYE_ORIGIN_VERIFIED_EPOCH ~= f11epoch then
        return false
    end
    return true
end
...
function Homelander_DualEyeFreeAimRenderProbe()
    ...
    if player == nil or target == nil or player == target or offset == nil or
       not finite(err) or err < 0.0 or not bound_to_current(player,target,offset,f9epoch,f11epoch) then
        log("ABORT: no valid same-player F9/F10/F11 proof")
        return false
    end

    local okEye, eye = pcall(go_GetJointPosition, player, "EYEPOINT")
    if not okEye or eye == nil or not bound_to_current(player,target,offset,f9epoch,f11epoch) then
    ...
    if not bound_to_current(player,target,offset,f9epoch,f11epoch) then
