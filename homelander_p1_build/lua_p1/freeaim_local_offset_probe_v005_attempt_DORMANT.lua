...
local function safe_mag(v)
    ...
    if not ok or not finite_number(m) or m < 0.0 then return nil end
    return m
end

local function safe_dot(a, b)
    local ok, d = pcall(function() return a:dot(b) end)
    if not ok or not finite_number(d) then return nil end
    return d
end
...
local function cosine_abs(a, b, aa, bb)
    ...
    local cosine = math.abs(d) / math.sqrt(aa * bb)
    if not finite_number(cosine) then return nil end
    return cosine
end
...
local function clear_f10_results()
    ...
    HOMELANDER_FREEAIM_F10_VERIFIED_ATTEMPT = nil
...
function Homelander_FreeAimLocalOffsetProbe()
    local previous = HOMELANDER_FREEAIM_F10_ATTEMPT
    ...
    if type(previous) ~= "number" or previous ~= previous or
       ...
       previous ~= math.floor(previous) then
        clear_f10_results()
        log("ABORT: invalid/overflowed F10 attempt generation")
        return false
    end
    local attempt = previous + 1
    HOMELANDER_FREEAIM_F10_ATTEMPT = attempt
    clear_f10_results()
    log("BEGIN — READ ONLY | F10 attempt=" .. tostring(attempt))
    local original_player = HOMELANDER_PLAYER
    local original_epoch = HOMELANDER_FREEAIM_VERIFIED_EPOCH
    if type(original_epoch) ~= "number" or original_epoch < 1 or
       original_epoch ~= original_epoch or
       HOMELANDER_FREEAIM_EPOCH ~= original_epoch then
        log("ABORT: F9 generation missing, invalid or already superseded")
        return false
    end
    ...
    local lx, ly, lz = dx / xx, dy / yy, dz / zz
    if not finite_number(lx) or not finite_number(ly) or not finite_number(lz) then
        log("ABORT: nonfinite local-offset projection")
        return false
    end
    local local_offset = Vector(lx, ly, lz)
    ...
    -- Logging and native transform callbacks may themselves invoke a newer F10
    -- on the SAME F9 hit and player. Only the most recent F10 attempt may win.
    if HOMELANDER_FREEAIM_F10_ATTEMPT ~= attempt then
        log("ABORT: newer F10 superseded this local-offset callback")
        return false
    end
    ...
    if HOMELANDER_PLAYER ~= original_player or not valid_goh(original_player) or
       ...
       HOMELANDER_FREEAIM_VERIFIED_EPOCH ~= original_epoch or
       HOMELANDER_FREEAIM_EPOCH ~= original_epoch or
       HOMELANDER_FREEAIM_HIT_POS ~= hit_pos or
       HOMELANDER_FREEAIM_HIT ~= target then
        log("ABORT: player/F9 target changed during F10 callbacks")
        return false
    end
    if HOMELANDER_FREEAIM_F10_ATTEMPT ~= attempt then
        log("ABORT: nested F10 during validity checks superseded local offset")
        return false
    end
    ...
    HOMELANDER_FREEAIM_F10_EPOCH = original_epoch
    HOMELANDER_FREEAIM_F10_VERIFIED_ATTEMPT = attempt
