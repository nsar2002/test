...
local function bound_to_current(player, target, offset)
    if HOMELANDER_PLAYER ~= player or
       ...
       HOMELANDER_EYE_ORIGIN_VERIFIED_PLAYER ~= player or
       HOMELANDER_FREEAIM_TARGET_LOCAL_OFFSET_GOH ~= target or
       HOMELANDER_FREEAIM_TARGET_LOCAL_OFFSET ~= offset then
        return false
    end
    return true
end
...
function Homelander_DualEyeFreeAimRenderProbe()
    ...
    if player == nil or target == nil or player == target or offset == nil or
       not finite(err) or err < 0.0 or not bound_to_current(player,target,offset) then
        log("ABORT: no valid same-player F9/F10/F11 proof")
        return false
    end

    local okEye, eye = pcall(go_GetJointPosition, player, "EYEPOINT")
    if not okEye or eye == nil or not bound_to_current(player,target,offset) then
    ...
    if not bound_to_current(player,target,offset) then
