...
HOMELANDER_DAMAGEHIT_PAYLOAD_PLAYER = nil
HOMELANDER_DAMAGEHIT_PAYLOAD_EYE_EPOCH = nil
HOMELANDER_DAMAGEHIT_PAYLOAD_ATTEMPT = 0
HOMELANDER_DAMAGEHIT_PAYLOAD_VERIFIED_ATTEMPT = nil
...
function Homelander_DamageHitPayloadProbeV015()
    -- A newer same-player F21 can run synchronously within an engine getter.
    -- Never publish a superseded older payload afterward.
    local previous = HOMELANDER_DAMAGEHIT_PAYLOAD_ATTEMPT
    ...
    if type(previous) ~= "number" or previous ~= previous or
       ...
       previous ~= math.floor(previous) then
        log("REFUSED: invalid/overflowed F21 payload generation")
        return false
    end
    local attempt = previous + 1
    HOMELANDER_DAMAGEHIT_PAYLOAD_ATTEMPT = attempt
    HOMELANDER_DAMAGEHIT_PAYLOAD_PASSED = false
    HOMELANDER_DAMAGEHIT_PAYLOAD_TARGET = nil
    HOMELANDER_DAMAGEHIT_PAYLOAD_SOURCE = nil
    HOMELANDER_DAMAGEHIT_PAYLOAD_HIT_POS = nil
    ...
    if HOMELANDER_TARGET_CONTINUITY_TARGET ~= target or
       ...
       HOMELANDER_DOT_DRYRUN_PLAYER ~= player or
       HOMELANDER_TARGET_CONTINUITY_PASSED ~= true or
       HOMELANDER_IMPACT_VFX_SUBMITTED ~= true then
        log("REFUSED: F21 lacks exact F17/F18/F19/F20 player and target lineage")
        return false
    end
    ...
    local eye_epoch = HOMELANDER_EYE_ORIGIN_VERIFIED_EPOCH
    if type(eye_epoch) ~= "number" or eye_epoch < 1 or
       eye_epoch ~= eye_epoch or
       HOMELANDER_EYE_ORIGIN_EPOCH ~= eye_epoch then
        log("REFUSED: current F11 eye generation not verified")
        return false
    end
    ...
    -- Every native getter (including health/validity) can synchronously run
    -- another F21 or another F11 on the same player. Re-read all source tokens.
    if HOMELANDER_DAMAGEHIT_PAYLOAD_ATTEMPT ~= attempt or
       ...
       HOMELANDER_DAMAGE_LAST_TARGET ~= target or
       HOMELANDER_IMPACT_VFX_SUBMITTED ~= true or
       HOMELANDER_TARGET_CONTINUITY_PASSED ~= true or
       HOMELANDER_DOT_DRYRUN_PASSED ~= true or
       HOMELANDER_EYE_ORIGIN_EPOCH ~= eye_epoch or
       HOMELANDER_EYE_ORIGIN_VERIFIED_EPOCH ~= eye_epoch or
       ...
       HOMELANDER_DAMAGEHIT_PAYLOAD_ATTEMPT ~= attempt or
       HOMELANDER_PLAYER ~= player or
       HOMELANDER_EYE_ORIGIN_EPOCH ~= eye_epoch then
        log("REFUSED: F21 source/eye/payload generation superseded in native getters")
        return false
    end
    HOMELANDER_DAMAGEHIT_PAYLOAD_PLAYER = player
    HOMELANDER_DAMAGEHIT_PAYLOAD_EYE_EPOCH = eye_epoch
    HOMELANDER_DAMAGEHIT_PAYLOAD_VERIFIED_ATTEMPT = attempt
