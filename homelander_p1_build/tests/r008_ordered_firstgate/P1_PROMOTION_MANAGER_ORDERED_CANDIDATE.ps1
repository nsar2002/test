...
# R008 CANDIDATE ONLY — temporal validation of the FIRST read-only live gate.
# This cannot authenticate a fabricated log; it only rejects unordered/stale
# matches accidentally combined across separate READY/F9 attempts.
function Assert-OrderedV003FirstGate([string]$Raw, $Cfg) {
    $lines = [regex]::Split($Raw, '\r?\n')
    $readyRx = [string]$Cfg.required[0]
    $cameraRx = [string]$Cfg.required[2]
    $hitRx = [string]$Cfg.required[3]
    $readyIndex = -1
    for ($i = 0; $i -lt $lines.Length; $i++) {
        if ([regex]::IsMatch($lines[$i], $readyRx)) { $readyIndex = $i }
    }
    if ($readyIndex -lt 0) { throw 'FIRST-GATE REFUSED: no matching READY build' }
    $f4Index = -1
    $lastF9 = -1
    for ($i = $readyIndex + 1; $i -lt $lines.Length; $i++) {
        if ($lines[$i] -match 'F4: read-only discovery \+ flight math probe') { $f4Index = $i }
        if ($lines[$i] -match 'F9: read-only free-aim camera/LOS probe') { $lastF9 = $i }
    }
    if ($f4Index -lt 0 -or $lastF9 -le $f4Index) {
        throw 'FIRST-GATE REFUSED: F4 and latest F9 dispatch are missing or out of order after latest READY'
    }
    $cameraIndex = -1
    $hitIndex = -1
    for ($i = $lastF9 + 1; $i -lt $lines.Length; $i++) {
        if ($cameraIndex -lt 0 -and [regex]::IsMatch($lines[$i], $cameraRx)) {
            $cameraIndex = $i
        }
        if ($cameraIndex -ge 0 -and [regex]::IsMatch($lines[$i], $hitRx)) {
            $hitIndex = $i
            break
        }
        if ($lines[$i] -match '\[HOMELANDER_P1_FREEAIM_V004\] MISS \|') {
            throw 'FIRST-GATE REFUSED: latest F9 is a miss, not a proven object hit'
        }
    }
    if ($cameraIndex -lt 0 -or $hitIndex -le $cameraIndex) {
        throw 'FIRST-GATE REFUSED: latest F9 lacks ordered CAMERA then non-nil HIT in current READY segment'
    }
    Write-Host 'FIRST-GATE ORDERED LOG PASS: latest READY -> F4 -> latest F9 -> CAMERA -> non-nil HIT.'
}
...
if ($Action -eq 'Check') {
    ...
    if ($Stage -eq 'v003') { Assert-OrderedV003FirstGate $raw $cfg }
