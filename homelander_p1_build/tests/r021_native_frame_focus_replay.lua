...
HOMELANDER_PLAYER=nil -- do not bypass original F4 acquisition at native load
...
-- Reproduce actual native F4: re-execute the original runtime Lua after the
-- 22 definition files have loaded, with generic engine-provided mock PLAYER.
-- The user\'s REAL v001 log shows PLAYER=nil; this is an offline positive
-- control, NOT evidence that the real game exposes this handle or passes v003.
PLAYER=nil
ai_GetPhysicsVelocity=function(h) eq(h,"A","mock verified velocity owner");return Vector(0,0,0) end
online_GetNumPlayers=function() return 1 end
online_DeterminePlayerIndex=function(h) if h=="A" then return 0 end return -1 end
dofile(stage.."/lua_p1/runtime_probe.lua")
eq(HOMELANDER_PLAYER,nil,"no mock PLAYER must mean original F4 cannot acquire player")
assert(not Homelander_FreeAimProbe(),"native F9 must fail closed after real-source F4 without player")
PLAYER="A"
dofile(stage.."/lua_p1/runtime_probe.lua")
eq(HOMELANDER_PLAYER,"A","original F4 acquired independently verified MOCK PLAYER")
print("P1_R021_REAL_SOURCE_F4_REQUIRED_BEFORE_NATIVE_F9_CHAIN_PASS")
...
for i=1,120 do
 assert(Homelander_LaserSightHeldTickV009(),"F15 native-order held frame "..i)
 assert(not Homelander_DynamicAimTickV010(),"native F16 tick must remain a no-op before F16 key")
end
...
for i=1,120 do
 assert(not Homelander_LaserSightHeldTickV009(),"native F15 must be off after F16 takes over")
 assert(Homelander_DynamicAimTickV010(),"F16 native-order dynamic frame "..i)
end
...
for i=1,15 do
 assert(Homelander_TargetContinuityTickV013(),"F19 native-order read-only sample "..i)
 assert(not Homelander_DOTDryRunTickV014(),"F20 native tick must be no-op before F20 key")
end
...
for i=1,35 do
 ...
 assert(not Homelander_TargetContinuityTickV013(),"F19 native tick must be no-op after F19 success")
 assert(Homelander_DOTDryRunTickV014(),"F20 native-order dryrun mock frame "..i)
end
...
-- Native C++ BridgeTick loses foreground focus -> FORCE DISABLE F15,F16,F19,F20
-- (followed by no per-frame active ticks). A new foreground session must not
-- re-submit laser, damage or VFX autonomously from old enabled Lua state.
local before_laser=laser_one_shot+laser_held+laser_dynamic
local before_damage=damage_calls
local before_effect=effect_calls
assert(Homelander_LaserSightHeldForceDisableV009())
assert(Homelander_DynamicAimForceDisableV010())
assert(Homelander_TargetContinuityForceDisableV013())
assert(Homelander_DOTDryRunForceDisableV014())
for i=1,10 do
 assert(not Homelander_LaserSightHeldTickV009(),"background F15 callback disabled")
 assert(not Homelander_DynamicAimTickV010(),"background F16 callback disabled")
 assert(not Homelander_TargetContinuityTickV013(),"background F19 callback disabled")
 assert(not Homelander_DOTDryRunTickV014(),"background F20 callback disabled")
end
eq(laser_one_shot+laser_held+laser_dynamic,before_laser,"focus loss must prevent new laser submissions")
eq(damage_calls,before_damage,"focus loss must prevent new damage")
eq(effect_calls,before_effect,"focus loss must prevent new VFX")
print("P1_R021_NATIVE_FOREGROUND_LOSS_F15_F16_F19_F20_FORCE_DISABLE_FAILCLOSED_PASS")
print("P1_R021_ACTUAL_NATIVE_FRAME_ORDER_F9_F21_MOCK_PASS")
