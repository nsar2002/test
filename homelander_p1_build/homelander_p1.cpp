...
namespace hl
{
    ...
    constexpr const char* kBuildId = "P1_RuntimeProbe_016_EFFECT_CATALOG_STAGED_20260919";
    ...
    bool g_vfxCandidateReady = false;
    ...
    void InstallLuaBridges(int L)
    {
        ...
        LuaPushCClosure(L, reinterpret_cast<void*>(&LuaLaserSightResetDynamicGateHook), 0);
        LuaSetField(L, LUA_GLOBALSINDEX, "HL_LaserSightResetDynamicGate");

        LuaPushCClosure(L, reinterpret_cast<void*>(&LuaLaserSightDynamicStabilityProbeHook), 0);
        LuaSetField(L, LUA_GLOBALSINDEX, "HL_LaserSightDynamicStabilityProbe");

        Log("Installed Lua bridges: HL_Log, HL_GetCameraFrame, HL_LastRayHitGOH, HL_LaserSightShaderProbe, HL_LaserSightSubmitDual, HL_LaserSightResetHeldGate, HL_LaserSightHeldStabilityProbe, HL_LaserSightResetDynamicGate, HL_LaserSightDynamicStabilityProbe");
    }
    ...
    void BootstrapLuaState(int L)
    {
        ...
        g_vfxCandidateReady = ExecuteLuaFile(L, "lua_p1\\vfx_candidate_resolver_v017_STAGED.lua");
        ...
        Log("Lua bootstrap scriptsReady=%s heatProbe=%s freeAim=%s localOffset=%s eyeOrigin=%s dualEyeRender=%s laserSightNative=%s heldLaserSight=%s dynamicAim=%s damageOneShot=%s impactVfx=%s targetContinuity=%s dotDryRun=%s damageHitPayload=%s targetVfxRoute=%s vfxCandidate=%s",
            ...
            g_vfxCandidateReady ? "true" : "false");
    }
    ...
    void BridgeTick(int L)
    {
        ...
        if (RisingEdge(VK_F19, g_f19Prev, inputEnabled))
        {
            if (g_targetContinuityReady)
            {
                Log("F19: toggle read-only target continuity/health telemetry");
                CallLua0(L, "Homelander_TargetContinuityToggleV013");
            }
            else
            {
                Log("F19 ignored: target continuity staged Lua did not load");
            }
        }
        ...
        if (RisingEdge(VK_F21, g_f21Prev, inputEnabled))
        {
            if (g_damageHitPayloadReady)
            {
                Log("F21: one-shot READ-ONLY DamageAndHit payload probe");
                CallLua0(L, "Homelander_DamageHitPayloadProbeV015");
            }
            else
            {
                Log("F21 ignored: DamageAndHit payload staged Lua did not load");
            }
        }

        if (RisingEdge(VK_F22, g_f22Prev, inputEnabled))
        {
            if (g_targetVfxRouteReady)
            {
                Log("F22: one-shot READ-ONLY target classification / VFX route probe");
                CallLua0(L, "Homelander_TargetVFXRouteProbeV016");
            }
            else
            {
        ...
        {
            if (g_vfxCandidateReady)
            {
                Log("F23: one-shot READ-ONLY exact VFX candidate resolver");
                CallLua0(L, "Homelander_VFXCandidateResolveV017");
            }
            else
            {
                Log("F23 ignored: exact VFX candidate staged Lua did not load");
            }
        }
