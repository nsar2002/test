...
## Separate dormant read-only proof-of-concept
Source lua_p1/grab_state_v01_DORMANT.lua defines Homelander_GrabStateProbeV01 as explicit-call-only. It samples ONLY the confirmed single-input bool go_IsGrabbed(current verified player), preserves the raw true/false without interpreting its gameplay meaning, revalidates the originating handle after the callback and clears stale state on failure. It never calls the two-argument getters with guessed slot strings and never attaches/detaches a game object. Offline Lua 5.1 test tests raw false/true, invalid/changed player, absent read API, non-boolean result, callbacks throwing and zero guessed getter/mutator calls. This code is NOT wired to any C++ hotkey and is not in the active game root.
